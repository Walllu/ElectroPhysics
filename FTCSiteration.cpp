...
bool Heat2D_next_u(Grid& next_u, Grid& prev_u, Stencil& stencil, const double alpha, float dt, double tolerate){
    ...
    for(int j=0;j<x_ext;j++){
		...
		if(abs(u_j_k_n1 - u_j_k_n) > largest_change){
	      largest_change = abs(u_j_k_n1 - u_j_k_n);
	    }
